...
docker run --rm \
  ...
  -v "${REPO_ROOT}:/workspace/cerlab-uav-sim:ro" \
  ...
  bash -lc '
    ...
    rsync -a --delete /workspace/cerlab-uav-sim/ /workspace/work/src/xgc2_cerlab_uav_simulator/
    ...
    /workspace/cerlab-uav-sim/.xgc2/scripts/package_debs.sh \
      --install-root /workspace/work/install-root \
      --output-dir /workspace/out

    if [[ "${INSTALL_CHECK}" == "true" ]]; then
      dpkg -i /workspace/out/*.deb
      /workspace/cerlab-uav-sim/.xgc2/scripts/check_installed_packages.sh
    fi
  '
