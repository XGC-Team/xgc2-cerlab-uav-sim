...
#include <xgc2_cerlab_uav_simulator/livox_lidar/livox_points_plugin.h>
...
#include <ignition/math/Vector3.hh>
#include <xgc2_cerlab_uav_simulator/LivoxCustomMsg.h>
#include <limits>
#include <xgc2_cerlab_uav_simulator/livox_lidar/csv_reader.hpp>
#include <xgc2_cerlab_uav_simulator/livox_lidar/livox_ode_multiray_shape.h>
#include <xgc2_cerlab_uav_simulator/livox_lidar/livox_point_xyzrtl.h>
...
void LivoxPointsPlugin::Load(gazebo::sensors::SensorPtr _parent, sdf::ElementPtr sdf) {
    ...
    const std::string package_uri = "package://xgc2_cerlab_uav_simulator/";
    if (file_name.rfind(package_uri, 0) == 0) {
        const auto package_path = ros::package::getPath("xgc2_cerlab_uav_simulator");
        file_name = package_path + "/" + file_name.substr(package_uri.size());
    } else if (!file_name.empty() && file_name.front() != '/') {
        const auto package_path = ros::package::getPath("xgc2_cerlab_uav_simulator");
        file_name = package_path + "/src/livox_lidar/scan_mode/" + file_name;
    }
    ...
    switch (publishPointCloudType) {
        ...
        case SENSOR_MSG_POINT_CLOUD2_LIVOXPOINTXYZRTLT:
            ...
            break;
        case xgc2_cerlab_uav_simulator_LIVOX_CUSTOM_MSG:
            rosPointPub = rosNode->advertise<xgc2_cerlab_uav_simulator::LivoxCustomMsg>(curr_scan_topic, 5);
            break;
        default:
            break;
    }
...
void LivoxPointsPlugin::PublishLivoxROSDriverCustomMsg(std::vector<std::pair<int, AviaRotateInfo>> &points_pair) {
    ...
    xgc2_cerlab_uav_simulator::LivoxCustomMsg msg;
    ...
    for (int i = 0; i < points_pair.size(); ++i) {
        ...
        if (verticle_index < verticalRayCount && horizon_index < rayCount) {
            ...
            auto point = range * axis;
            // pt.intensity = static_cast<float>(intensity);
            xgc2_cerlab_uav_simulator::CustomPoint pt;
            pt.x = point.X();
            pt.y = point.Y();
            pt.z = point.Z();
            pt.line = pair.second.line;
